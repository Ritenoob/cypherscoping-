...
app.use((req, res, next) => {
  ...
  // Content Security Policy - Allow unsafe-eval for dashboard functionality
...
// ============================================================================
// DEMO TRADING ENGINE - Paper Trading System
// ============================================================================
const demoTradingEngine = new DemoTradingEngine({
  initialBalance: 10000,    // $10k paper money
  maxPositions: 3,          // Max 3 concurrent positions
  maxRiskPerTrade: 2.0      // 2% risk per trade
});

// Forward demo trading events to dashboard
demoTradingEngine.on('position_opened', (position) => {
  broadcast({ type: 'demo_position_opened', position });
  broadcastLog('success', `[DEMO] Opened ${position.symbol} ${position.side.toUpperCase()} @ $${position.entryPrice.toFixed(2)}`);
});

demoTradingEngine.on('position_closed', (trade) => {
  broadcast({ type: 'demo_position_closed', trade });
  const pnl = trade.realizedPnl >= 0 ? `+$${trade.realizedPnl.toFixed(2)}` : `-$${Math.abs(trade.realizedPnl).toFixed(2)}`;
  broadcastLog('info', `[DEMO] Closed ${trade.symbol} ${trade.reason}: ${pnl} (${trade.realizedPnlPercent.toFixed(2)}%)`);
});
...
function broadcastMarketData(symbol) {
  ...
  // Feed market data to demo trading engine
  demoTradingEngine.onMarketUpdate(symbol, {
    marketData,
    indicators,
    signal
  });
...
async function executeEntry(symbol, side, positionSizePercent = CONFIG.TRADING.POSITION_SIZE_PERCENT, leverage = CONFIG.TRADING.DEFAULT_LEVERAGE) {
  ...
  // ========================================================================
  // V3.5.2: PHASE 2 - COMPREHENSIVE ENTRY VALIDATION
  // ========================================================================
  ...
  // ========================================================================
  // V3.5.2: PHASE 2 - POSITION SIZE LIMIT VALIDATION
  // ========================================================================
...
app.post('/api/demo/toggle', (req, res) => {
  ...
  demoTradingEngine.setEnabled(enabled);
  res.json({
    success: true,
    enabled: demoTradingEngine.enabled
  });
});

// Get demo trading status and metrics
app.get('/api/demo/status', (req, res) => {
  const metrics = demoTradingEngine.getMetrics();
  const positions = Array.from(demoTradingEngine.positions.values());

  res.json({
    enabled: demoTradingEngine.enabled,
    metrics,
    positions,
    params: demoTradingEngine.params
  });
});

// Reset demo account
app.post('/api/demo/reset', (req, res) => {
  demoTradingEngine.reset();
  res.json({ success: true });
});

// Get demo trade history
app.get('/api/demo/trades', (req, res) => {
  const limit = parseInt(req.query.limit) || 50;
  const trades = demoTradingEngine.trades.slice(-limit);
  res.json({ trades });
});

// Update demo trading parameters
app.post('/api/demo/params', (req, res) => {
  const { params } = req.body;
  if (params) {
    Object.assign(demoTradingEngine.params, params);
    broadcastLog('info', '[DEMO] Parameters updated', params);
  }
  res.json({
    success: true,
    params: demoTradingEngine.params
...
function startIntervals() {
  console.log('[DEBUG] Starting intervals...');
  // Update market data every 3 seconds
  intervalRefs.push(setInterval(async () => {
    console.log('[DEBUG] Market data interval running...');
    for (const symbol of Object.keys(marketManagers)) {
      await fetchTicker(symbol);
      broadcastMarketData(symbol);
      await sleep(100);
