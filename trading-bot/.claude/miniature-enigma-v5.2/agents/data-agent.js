...
class DataAgent extends AgentBase {
  ...
  async fetchCandles(symbol, timeframe, limit = 200) {
    ...
    // Update ticker cache with latest candle price
    if (candles.length > 0) {
  ...
  /**
   * Fetch order book - uses cache when rate limited
   */
  async fetchOrderBook(symbol, depth = 20) {
    const result = await this._apiRequest('GET', `/api/v1/level2/snapshot?symbol=${symbol}`);

    // If rate limited or error, try to use cached data
    if (!result.ok) {
      const cached = this.orderBookCache.get(symbol);
      if (cached && (Date.now() - cached.timestamp) < 60000) { // Cache valid for 60s
        return { ok: true, value: cached, fromCache: true };
      }
      // If no cache, generate synthetic orderbook from last known price
      const ticker = this.tickerCache.get(symbol);
      if (ticker) {
        const syntheticBook = this._generateSyntheticOrderBook(symbol, ticker.price, depth);
        return { ok: true, value: syntheticBook, synthetic: true };
      }
      return result;
    }
  ...
  /**
   * Generate synthetic orderbook from price (fallback when rate limited)
   */
  _generateSyntheticOrderBook(symbol, midPrice, depth = 20) {
    const spread = midPrice * 0.0005; // 0.05% spread
    const bids = [];
    const asks = [];
