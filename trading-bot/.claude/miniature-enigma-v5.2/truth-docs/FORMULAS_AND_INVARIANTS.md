...
```javascript
// F-028: Stochastic RSI
// StochRSI = Stoch(RSI) - feeds RSI values into Stochastic formula
// Your settings: StochRSI 50 14 4 5 (rsiPeriod=50, stochPeriod=14, kSmooth=4, dSmooth=5)
function calculateStochRSI(closes, rsiPeriod = 50, stochPeriod = 14, kSmooth = 4, dSmooth = 5) {
  // Step 1: Compute RSI history
  const rsiHistory = [];
  for (let i = rsiPeriod; i <= closes.length; i++) {
    rsiHistory.push(calculateRSI(closes.slice(0, i), rsiPeriod));
  }
  ...
  for (let i = stochPeriod - 1; i < rsiHistory.length; i++) {
    ...
    const denom = highestRSI - lowestRSI;
    const stochRsi = denom === 0 ? 50 : ((rsiHistory[i] - lowestRSI) / denom) * 100;
    stochRsiValues.push(stochRsi);
  }
  
  // Step 3: Smooth to get %K (SMA of stochRsi)
  const kValues = [];
  for (let i = kSmooth - 1; i < stochRsiValues.length; i++) {
    const k = stochRsiValues.slice(i - kSmooth + 1, i + 1).reduce((a, b) => a + b, 0) / kSmooth;
    kValues.push(k);
  }
...
function calculateCMF(highs, lows, closes, volumes, period = 20) {
  ...
  for (let i = closes.length - period; i < closes.length; i++) {
    ...
    // Simplifies to: (2 * Close - Low - High) / (High - Low)
...
// Interpretation:
// - CMF > +0.25: Strong buying pressure (bullish)
// - CMF > +0.10: Moderate buying pressure (bullish)
// - CMF < -0.25: Strong selling pressure (bearish)
// - CMF < -0.10: Moderate selling pressure (bearish)
// - CMF crosses above 0: Bullish signal
// - CMF crosses below 0: Bearish signal
// - Price rising + CMF falling: Bearish divergence
// - Price falling + CMF rising: Bullish divergence
```

### 3.6 OBV with WMA/SMA Smoothing

```javascript
// F-030: On-Balance Volume with Smoothing
// OBV base calculation + WMA/SMA overlays for signal confirmation
function calculateOBVWithSmoothing(closes, volumes, wmaPeriod = 20, smaPeriod = 20) {
  ...
  // WMA of OBV (more weight to recent values)
  ...
  const obvWMA = wma(obvHistory, wmaPeriod);
  const obvSMA = sma(obvHistory, smaPeriod);
  
  return { obv, obvHistory, obvWMA, obvSMA };
}

// Interpretation:
// - OBV > WMA: Bullish (volume supporting price)
// - OBV < WMA: Bearish (volume not supporting price)
// - OBV crosses above WMA: Buy signal
// - OBV crosses below WMA: Sell signal
// - SMA provides secondary confirmation
```
...
// F-027: Signal Classification (adjusted for 13 indicators, max ±160)
function classifySignal(score) {
  ...
  if (score > -100) return 'STRONG_SELL';
  return 'EXTREME_SELL';
}
```
