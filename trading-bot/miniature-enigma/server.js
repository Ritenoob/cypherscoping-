...
class TechnicalIndicators {
  ...
  static calculateEMA(data, period) {
    const series = this.calculateEMASeries(data, period);
    if (!series || series.length === 0) return null;
    return series[series.length - 1];
  }

  // Returns a full EMA series aligned to the input data indices.
  // Values before the first EMA seed are null.
  static calculateEMASeries(data, period) {
    if (!data || data.length < period) return null;
    const multiplier = 2 / (period + 1);

    const series = new Array(data.length).fill(null);
    const seed = this.calculateSMA(data.slice(0, period), period);
  ...
  static calculateRSI(data, period = 14) {
    // Wilder's RSI (what most platforms use)
  ...
  static calculateMACD(data, fastPeriod = 12, slowPeriod = 26, signalPeriod = 9) {
    // Standard MACD: EMA(fast) - EMA(slow); signal = EMA(macd, signalPeriod)
    if (!data || data.length < slowPeriod + signalPeriod) {
      return { macd: 0, signal: 0, histogram: 0 };
    }

    const fastSeries = this.calculateEMASeries(data, fastPeriod);
    const slowSeries = this.calculateEMASeries(data, slowPeriod);
    if (!fastSeries || !slowSeries) return { macd: 0, signal: 0, histogram: 0 };
    ...
    if (macdValues.length < signalPeriod) {
      const lastMacd = macdValues.length ? macdValues[macdValues.length - 1] : 0;
      return { macd: lastMacd, signal: 0, histogram: lastMacd };
    }

    const signalSeries = this.calculateEMASeries(macdValues, signalPeriod);
    const macd = macdValues[macdValues.length - 1] || 0;
    const signal = (signalSeries && signalSeries[signalSeries.length - 1] !== null)
  ...
  static calculateStochastic(highs, lows, closes, period = 14, smoothK = 3, smoothD = 3) {
    // Standard Stochastic oscillator with smoothing:
    ...
    for (let i = period - 1; i < closes.length; i++) {
      ...
      if (windowHigh === windowLow) {
        ...
        rawK.push(((closes[i] - windowLow) / (windowHigh - windowLow)) * 100);
      }
    }
    ...
    for (let i = 0; i < rawK.length; i++) {
      if (i + 1 >= smoothK) {
        smoothedK.push(sma(rawK.slice(i - smoothK + 1, i + 1)));
      }
    }

    if (smoothedK.length === 0) {
      const lastRaw = rawK.length ? rawK[rawK.length - 1] : 50;
      return { k: lastRaw, d: lastRaw };
    }
...
class MarketDataManager {
  ...
  updateTicker(tickerData) {
    // Keep current price updated from ticker, and also update the most recent
    // candle's OHLC so indicators can move intrabar (matches most platforms).
    if (tickerData.price) {
      ...
      if (!Number.isNaN(p)) {
        ...
        if (last) {
          // Treat latest candle as "in-progress" and update OHLC in-place.
          last.close = p;
          if (typeof last.high === 'number') last.high = Math.max(last.high, p);
          if (typeof last.low === 'number') last.low = Math.min(last.low, p);
        }
      }
    }
...
async function fetchKlines(symbol, timeframe = currentTimeframe, candleLimit = 500) {
  try {
    const granularity = CONFIG.TIMEFRAMES[timeframe] || 5;
    const to = Date.now();
    const safeLimit = Math.max(60, Math.min(600, Number(candleLimit) || 500));
    const from = to - (granularity * safeLimit * 60 * 1000);
...
// Refresh candles periodically so indicator values track the platform over time.
// Between refreshes, updateTicker() keeps the most recent candle moving intrabar.
// NOTE: We intentionally refresh fewer candles than the full 500 to reduce API load.
const KLINE_REFRESH_INTERVAL_MS = 30000; // 30s
const KLINE_REFRESH_CANDLES = 350;       // Enough for EMA200 + buffer

setInterval(async () => {
  for (const symbol of Object.keys(marketManagers)) {
    await fetchKlines(symbol, currentTimeframe, KLINE_REFRESH_CANDLES);
    await sleep(150);
  }
