...
export class WilliamsRIndicator extends EventEmitter {
  ...
  private fastHighs: number[] = [];
  private fastLows: number[] = [];
  private wrHistory: number[] = [];
  private priceHistory: number[] = [];
  private maxHistory: number;

  private oversoldBars: number = 0;
  private overboughtBars: number = 0;
  private currentValue: number | null = null;
  private prevValue: number | null = null;
  private fastValue: number | null = null;
  private prevFastValue: number | null = null;

  constructor(config: { period?: number; fastPeriod?: number; oversold?: number; overbought?: number; historyLength?: number } = {}) {
    super();
  ...
  update(candle: { high: number; low: number; close: number }): WilliamsRResult {
  ...
  private getCrossover(): SignalResult | null {
  ...
  private getFailureSwing(): SignalResult | null {
  ...
  private getDivergence(): SignalResult | null {
  ...
  private getHiddenDivergence(): SignalResult | null {
    if (this.wrHistory.length < 20) return null;
  ...
  private getZone(): SignalResult | null {
  ...
  private getMomentumThrust(): SignalResult | null {
  ...
  private getHookPattern(): SignalResult | null {
  ...
  private getFastSlowCrossover(): SignalResult | null {
    if (this.fastValue === null || this.prevFastValue === null || this.currentValue === null || this.prevValue === null) return null;
  ...
  private findSwingLows(data: number[]): number[] {
    const lows = [];
  ...
  private findSwingHighs(data: number[]): number[] {
    const highs = [];
    for (let i = 2; i < data.length - 2; i++) {
      if (data[i] > data[i - 1] && data[i] > data[i - 2] &&
          data[i] > data[i + 1] && data[i] > data[i + 2]) {
